<template>
  ...
  <div class="container mx-auto">
    <h1 class="mb-4 text-4xl font-semibold text-white">Statistics summary</h1>
    <div class="flex flex-row justify-center w-full">
      <div class="shadow stats stats-vertical bg-base-300 lg:stats-horizontal">
        <div class="stat">
          ...
          <router-link
            custom
            :to="{ name: 'Developer Users' }"
            v-slot="{ navigate }"
          >
            <div
              class="cursor-pointer stat-value text-primary"
              :title="summaryResponse.totalUsers.toString() || '0'"
              @click="navigate"
            >
        ...
        <div class="stat">
          ...
          <router-link
            custom
            :to="{ name: 'Developer Events' }"
            v-slot="{ navigate }"
          >
            <div
              class="cursor-pointer stat-value text-primary"
              :title="summaryResponse.totalEvents.toString() || '0'"
              @click="navigate"
            >
        ...
        <div class="place-items-center stat">
          ...
          <div
            ...
            :title="summaryResponse.newUsersThisMonth.toString() || '0'"
          >
            {{ numberFormat.format(summaryResponse.newUsersThisMonth) }}
          </div>
          <div
            class="stat-desc"
            :title="`last month: ${summaryResponse.newUsersPastMonth}`"
          >
            {{
              d3.format("+0.4")(
                summaryResponse.newUsersPercentageDifferenceToLastMonth,
              )
            }}% from last month
          </div>
        </div>
        <div class="place-items-center stat">
          ...
          <div
            class="stat-value text-primary"
            :title="summaryResponse.newEventsThisMonth.toString() || '0'"
          >
            {{ numberFormat.format(summaryResponse.newEventsThisMonth) }}
          </div>
          <div
            class="stat-desc"
            :title="`last month: ${summaryResponse.newEventsPastMonth}`"
          >
            {{
              d3.format("+0.4")(
                summaryResponse.newEventsPercentageDifferenceToLastMonth,
              )
            }}% from last month
          </div>
        </div>
        <div class="place-items-center stat">
          <div class="stat-title">Total gross income (THB)</div>
          <div
            class="stat-value text-primary"
            :title="summaryResponse.totalGrossIncome.toString() || '0'"
          >
    ...
    <div class="flex flex-row gap-3 mt-8">
      <div class="dropdown">
        ...
        <ul
          tabindex="0"
          class="p-2 w-52 text-white shadow dropdown-content menu bg-base-200 rounded-box"
        >
          <li>
      ...
      <h1 class="text-4xl font-semibold text-white">origin map between</h1>
    ...
    <div class="overflow-x-auto mt-8">
      <table
        ...
        v-show="worldMapResponse[selectedChartType].length !== 0"
      >
        <thead>
          <tr>
            <th>Country</th>
        ...
        <tbody>
          <tr
            v-show="selectedChartType === 'users'"
            v-for="(u, i) in worldMapResponse.users"
            :key="`world-map-users-table-${i}`"
          >
            <th>{{ getCountryName(u.country) }} ({{ u.country }})</th>
            <td>
              {{ u.amount }}
            </td>
          </tr>
          <tr
            v-show="selectedChartType === 'events'"
            v-for="(e, i) in worldMapResponse.events"
            :key="`world-map-events-table-${i}`"
          >
            <th>{{ getCountryName(e.country) }} ({{ e.country }})</th>
            <td>
              {{ e.amount }}
            </td>
          </tr>
    ...
    <div class="flex flex-row gap-2 mt-8">
      ...
      <h1 class="text-4xl font-semibold text-white">group by</h1>
      <div class="dropdown">
        ...
        <ul
          tabindex="0"
          class="p-2 w-52 shadow dropdown-content menu bg-base-200 rounded-box"
        >
    ...
    <table class="table mt-4 w-full table-compact">
      ...
      <tbody>
        <tr
          v-for="(t, i) in topEventTagsOfAllTimeResponse.tags"
          :key="`top-event-tags-table-${i}`"
        >
          <th>
            {{ formatTagName(t.tag) }}
          </th>
  ...
  setup() {
    ...
    const createTransactionChart = (): void => {
      transactionChartSVG.value = d3
        .select("div#transaction-bar-chart")
        .append("svg")
    ...
    const createRegistrationHistoryChart = (): void => {
      registrationChartSVG.value = d3
        .select("div#registration-bar-chart")
        .append("svg")
