...
export const generateEventSections = (
  row: number,
  column: number,
  initialSeats: Array<Array<ReebAEventSeat>>
): Array<Array<ReebAEventSection>> => {
  if (row <= 0) {
  ...
  return Array.from<Array<ReebAEventSection>, Array<ReebAEventSection>>(
    { length: Math.floor(row) },
    (_, u) => {
      return Array.from<ReebAEventSection, ReebAEventSection>(
        { length: Math.floor(column) },
        (_, v) => {
          return {
...
export const generateEventSeats = (
  row: number,
  column: number,
  initialPrice: number
): Array<Array<ReebAEventSeat>> => {
  if (row <= 0) {
    throw new Error("property 'row' cannot be less than or equal to 0")
  }
  ...
  return Array.from<Array<ReebAEventSeat>, Array<ReebAEventSeat>>(
    { length: Math.floor(row) },
    (_, u) => {
      return Array.from<ReebAEventSeat, ReebAEventSeat>(
        { length: Math.floor(column) },
        (_, v) => {
          return {
            seatRowPosition: u,
...
export const increase2DArrayDimension = <T>(
  initialArray: Array<Array<T>>,
...
export const decrease2DArrayDimension = <T>(
  initialArray: Array<Array<T>>,
  direction: "row" | "column"
): Array<Array<T>> => {
  if (direction === "row") {
