...
const userPassword =
  "$argon2id$v=19$m=4096,t=12,p=4$snysIHKmfhvtO+uORE8jwA$KemeOraiG6zasIT8bgYljyEuoGHgl5RQAUhr9bRBTVhJ11sGaErGuA"
...
const generateUserList = async (amount: number): Promise<Array<users>> => {
  ...
  for await (const _ of [...range(amount)]) {
    ...
    const user: users = {
      ...
      user_registration_datetime: dayjs(
        faker.date.between("2020-01-01", "2021-01-01")
      ).toISOString(),
      user_role:
        faker.mersenne.rand(1, 100) > 90 ? t_user_role.admin : t_user_role.user,
      ...
      user_birthdate: dayjs(
        faker.date.between("1960-01-01", "2006-01-01")
      ).format("YYYY-MM-DD"),
...
const generateFollowersList = (
  userList: Array<users>,
  amount: number = 50
): Array<user_followers> => {
...
const generateEvent = async (
  ...
  // eslint-disable-next-line
  for await (const _ of [...range(amount)]) {
    const creationDateString = dayjs(
      faker.date.between(
    ...
    const reebaEvent: CustomEvent = {
      ...
      event_opening_date: dayjs(
        faker.date.soon(90, dayjs(creationDateString).format("YYYY-MM-DD"))
      )
        .set("minute", 0)
        .set("second", 0)
        .set("millisecond", 0)
      ...
      event_minimum_age:
        faker.mersenne.rand(1, 100) > 60
          ? faker.mersenne.rand(18, 20).toString()
          : "0",
  ...
  for (const indivEvent of eventList) {
    ...
    for (const _ of [...range(faker.mersenne.rand(2, 5))]) {
      const datetime: event_datetimes = {
        ...
        event_end_datetime: openingDate
          .add(7, "days")
          .add(2, "hours")
          .toISOString()
      }
  ...
  for (const indivEvent of eventList) {
    ...
    for (const indivDatetime of eventDatetimeList.filter(
      datetime => datetime.event_id === indivEvent.event_id
    )) {
      const sections = Array.from<Array<event_sections>, Array<event_sections>>(
        { length: sectionRowAmount },
        (_, i) => {
          return Array.from<event_sections, event_sections>(
            { length: sectionColumnAmount },
            (_, j) => {
              return {
  ...
  for (const indivEvent of eventList) {
    for (const indivDatetime of eventDatetimeList.filter(
      ...
      const seatRowAmount = Object.keys(
        indivEvent.prices_array ?? Object.keys({})
      ).length
      const seatColumnAmount = faker.mersenne.rand(8, 30)
      for (const indivSection of eventSectionList.filter(
        sec => sec.event_datetime_id === indivDatetime.event_datetime_id
      )) {
        const seats = Array.from<Array<event_seats>, Array<event_seats>>(
          { length: seatRowAmount },
          (_, i) => {
            return Array.from<event_seats, event_seats>(
              { length: seatColumnAmount },
              (_, j) => {
                return {
                  event_seat_id: compatibleExcelNanoid(),
                  event_section_id: indivSection.event_section_id,
                  event_seat_price: Object.values(
                    indivEvent.prices_array ?? {}
                  )[i],
...
const getSectionIdFromDatetimeArray = (
  datetimes: Array<event_datetimes>,
  ...
  for (const datetime of datetimes) {
    for (const section of eventData.sections.filter(
      sec => sec.event_datetime_id === datetime.event_datetime_id
    )) {
...
const getAllSeatsFromSectionId = (
  sectionId: string,
  eventData: EventGroup
): Array<event_seats> => {
  const seatsList: Array<event_seats> = eventData.seats.filter(
    seat => seat.event_section_id === sectionId
  )
  return seatsList
}
...
const generateTransactions = (
  ...
  const transactions = Array.from<transactions, transactions>(
    { length: 300 },
    () => {
      ...
      const lastDateEvent = eventData.datetimes
        ...
        .sort(
          (a, b) =>
            dayjs(b.event_start_datetime).unix() -
            dayjs(a.event_start_datetime).unix()
        )
  ...
  transactions.forEach(transact => {
    ...
    const buyableDatetimes = eventData.datetimes
      .filter(
        datetime =>
          dayjs(transact.transaction_time).unix() <
          dayjs(datetime.event_start_datetime).unix()
      )
      .filter(datetime => transact.event_id === datetime.event_id)

    const randomSectionId = getSectionIdFromDatetimeArray(
      buyableDatetimes,
      eventData
    )
    const allSeatsGroupedByPrice = groupBy(
      getAllSeatsFromSectionId(randomSectionId, eventData),
      s => s.event_seat_price
    )
    const priceKey = faker.helpers.arrayElement(
      Object.keys(allSeatsGroupedByPrice)
    )

    const selectedSeat = faker.helpers
      .arrayElements(
        allSeatsGroupedByPrice[parseInt(priceKey)],
        faker.mersenne.rand(1, 4)
      )
      .map(s => {
        return {
...
const generateEventTags = (): Array<event_tags> => {
  const tags: Array<event_tags> = [
    ...
    return { event_tag_label: t }
  })

  return tags
}

const generateTagsBridge = (
  tags: Array<event_tags>,
  events: Array<CustomEvent>
): Array<event_tags_bridge> => {
  const bridge: Array<event_tags_bridge> = []

  events.forEach(ev => {
    const tagForEv = faker.helpers.arrayElements(
      tags,
      faker.mersenne.rand(1, 5)
    )

    tagForEv.forEach(t =>
      bridge.push({ event_tag_label: t.event_tag_label, event_id: ev.event_id })
    )
  })
...
const main = async () => {
  ...
  const usersDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "users.csv")
  )
  ...
  const followersDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "user_followers.csv")
  )
  const followersStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const eventDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "events.csv")
  )
  const eventsStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const eventDatetimesDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "event_datetimes.csv")
  )
  const eventDatetimesStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const eventSectionsDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "event_sections.csv")
  )
  const eventSectionsStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const eventSeatsDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "event_seats.csv")
  )
  const eventSeatsStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const transactionsDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "transactions.csv")
  )
  const transactionsStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const transactionDetailsDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "transaction_details.csv")
  )
  const transactionsDetailsStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const eventTagsDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "event_tags.csv")
  )
  const eventTagsStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  const tagsBridgeDestinationFile = createWriteStream(
    resolve(__dirname, "..", "output", "event_tags_bridge.csv")
  )
  const tagsBridgeStream = csv.format({
    headers: true,
    delimiter: ",",
    quote: true
  })
  ...
  for await (const ev of eventData.event) {
    ...
    for await (const evTags of bridge.filter(
      b => eventId.rows[0].event_id === b.event_id
    )) {
      await client.query(
    ...
    for await (const dt of eventData.datetimes.filter(
      dt => dt.event_id === eventId.rows[0].event_id
    )) {
      const dtId = await client.query(
        ...
        [
          dt.event_datetime_id,
          eventId.rows[0].event_id,
          dayjs(dt.event_start_datetime).toDate(),
          dayjs(dt.event_end_datetime).toDate()
        ]
      )

      for await (const sec of eventData.sections.filter(
        sec => sec.event_datetime_id === dtId.rows[0].event_datetime_id
      )) {
        ...
        for await (const seat of eventData.seats.filter(
          seat => seat.event_section_id === secId.rows[0].event_section_id
        )) {
          await client.query(
  ...
  for await (const t of transactions.transactions) {
    await client.query(
      ...
      [t.transaction_id, t.user_username, dayjs(t.transaction_time).toDate()]
    )
  }
