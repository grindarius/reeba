...
void t.test("get front page event test", async t => {
  ...
  const isThereOfficialUser = await client.query(
  ...
  if (isThereOfficialUser.rowCount === 0) {
    await client.query(
      ...
      [
        officialUser.username,
        officialUser.email,
        officialUser.password,
        officialUser.phoneCountryCode,
  ...
  const isThereLocalUser = await client.query(
    'select * from "users" where user_username = $1',
    [localUser.username]
  )
