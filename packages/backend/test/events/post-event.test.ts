...
void t.test("post event", async t => {
  ...
  try {
    await client.query(
      "delete from \"events\" where user_username = 'postindiveventtest'"
    )
  ...
  const undefinedEventName = (({ eventName, ...others }) => ({ ...others }))(
    perfectEvent
  )

  const emptyStringCreatedBy = { ...perfectEvent, ...{ createdBy: "" } }
  const undefinedCreatedBy = (({ createdBy, ...others }) => ({ ...others }))(
    perfectEvent
  )

  const undefinedDescription = (({ description, ...others }) => ({
    ...others
  }))(perfectEvent)

  const undefinedWebsite = (({ website, ...others }) => ({ ...others }))(
    perfectEvent
  )

  const emptyStringVenueName = { ...perfectEvent, ...{ venueName: "" } }
  const undefinedVenueName = (({ venueName, ...others }) => ({ ...others }))(
    perfectEvent
  )

  const emptyObjectVenueCoordinates = {
    ...perfectEvent,
    ...{ venueCoordinates: {} }
  }
  const wrongKeyNameOfVenueCoordinates = {
    ...perfectEvent,
    ...{ venueCoordinates: { lat: 123, long: 456 } }
  }
  const wrongKeyNameOfVenueCoordinatesForY = {
    ...perfectEvent,
    ...{ venueCoordinates: { x: 123, long: 456 } }
  }
  const wrongTypeVenueCoordinates = {
    ...perfectEvent,
    ...{ venueCoordinates: { x: "2345", y: 1234 } }
  }
  const undefinedVenueCoordinates = (({ venueCoordinates, ...others }) => ({
    ...others
  }))(perfectEvent)

  const emptyStringOpeningDate = { ...perfectEvent, ...{ openingDate: "" } }
  const undefinedOpeningDate = (({ openingDate, ...others }) => ({
    ...others
  }))(perfectEvent)
  ...
  const undefinedTicketPrices = (({ ticketPrices, ...others }) => ({
    ...others
  }))(perfectEvent)

  const emptyStringDatetimes = { ...perfectEvent, ...{ datetimes: [] } }
  const containsEmptyStringDatetimes = {
    ...perfectEvent,
    ...{ datetimes: [{ start: "", end: "" }] }
  }
  const undefinedDatetimes = (({ datetimes, ...others }) => ({ ...others }))(
    perfectEvent
  )

  const negativeMinimumAge = { ...perfectEvent, ...{ minimumAge: -5 } }
  const undefinedMinimumAge = (({ minimumAge, ...others }) => ({ ...others }))(
    perfectEvent
  )

  const emptyArraySections = { ...perfectEvent, ...{ sections: [] } }
  const undefinedSections = (({ sections, ...others }) => ({ ...others }))(
    perfectEvent
  )
  ...
  void t.test("missing eventName (as empty string)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing eventNames"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'eventName'",
        "Error message from missing eventNames"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing createdBy (as empty string)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing createdBy"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'createdBy'",
        "Error message from missing createdBy"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing description (as missing params)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing description"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'description'",
        "Error message from missing description"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing website (as missing params)", async t => {
    try {
      ...
      t.strictSame(
        response.json().message,
        "body should have required property 'website'",
        "Error message from missing website"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing venueName (as empty string)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing venueName"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'venueName'",
        "Error message from missing venueName"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing venueCoordinates (as empty object)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing venueCoordinates"
      )
      t.strictSame(
        response.json().message,
        "venueCoordinates.x should be type of 'string'",
        "Error message from missing venueCoordinates"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test(
    "missing venueCoordinates (wrong key name) for y value",
    async t => {
      try {
        const response = await app.inject({
          method: "POST",
  ...
  void t.test("missing venueCoordinates (wrong type)", async t => {
    try {
      ...
      t.strictSame(
        response.json().message,
        "venueCoordinates.y should be type of 'string'",
        "Error message from missing venueCoordinates"
      )
    } catch (error) {
      t.error(error)
  ...
  void t.test("missing openingDate (as empty string)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing openingDate"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'openingDate'",
        "Error message from missing openingDate"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing tags (as missing params)", async t => {
    try {
      ...
      t.strictSame(
        response.json().message,
        "body should have required property 'tags'",
        "Error message from missing tags"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing ticketPrices (as empty array)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing ticketPrices"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'ticketPrices'",
        "Error message from missing ticketPrices"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing datetimes (as empty string)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing datetimes"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'datetimes'",
        "Error message from missing datetimes"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing minimumAge (age negative)", async t => {
    try {
      ...
      t.strictSame(
        response.statusCode,
        400,
        "Error code from missing minimumAge"
      )
      t.strictSame(
        response.json().message,
        "body should have required property 'minimumAge'",
        "Error message from missing minimumAge"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing sections (as empty array)", async t => {
    try {
      ...
      t.strictSame(
        response.json().message,
        "body should have required property 'sections'",
        "Error message from missing sections"
      )
    } catch (error) {
      t.error(error)
      t.fail()
  ...
  void t.test("missing seats (as missing params)", async t => {
    try {
      ...
      t.strictSame(
        response.json().message,
        "body.sections[0][0] should have required property 'seats'",
        "Error message from missing seats"
      )
    } catch (error) {
      t.error(error)
      t.fail()
