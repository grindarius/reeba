...
const mockEvent = async (): Promise<void> => {
  ...
  const targetUser = await client.query(
    "select * from \"users\" where user_username = 'getindiveventtest'"
  )
  ...
  const targetEvent = await client.query<events, [string]>(
  ...
  void t.test("get event with correct id", async t => {
    try {
      ...
      const json = response.json<{
        ...
        website: string
        venueName: string
        venueCoordinates: { x: string; y: string }
        openingDate: string
        tags: Array<string>
        datetimes: Array<{ datetimeId: string; start: string; end: string }>
      }>()

      t.strictSame(
        response.statusCode,
        200,
        "status code from correct response"
      )
      ...
      t.strictSame(json.venueCoordinates, {
        x: "13.755313892097984",
        y: "100.62221451070221"
      })
