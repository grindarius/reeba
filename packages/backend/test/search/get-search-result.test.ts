...
void t.test("search function test", async t => {
  ...
  await client.query(
    "delete from \"events\" where event_name in ('The rightful music', 'The leftful musicque')"
  )
  await client.query(
    "delete from \"users\" where user_username = 'thatrich_test_guy'"
  )
  ...
  void t.test(
    "searching for events with date range for events date not exists",
    async t => {
      try {
        const response = await app.inject({
          method: "get",
