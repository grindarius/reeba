...
void t.test('transferring a ticket to a user', async t => {
  ...
  const tid = await client.query('select * from "transactions" where user_username = $1', ['dripped_out'])
  const tid2 = await client.query('select * from "transactions" where user_username = $1', ['another_rich_dude_lol'])
  ...
  await client.query('delete from "events" where user_username = $1', ['eventowneryeah'])
  await client.query('delete from "users" where user_username in ($1, $2, $3, $4)', ['eventowneryeah', 'firstbuyer', 'dripped_out', 'another_rich_dude_lol'])
  ...
  const anotherBuyerTryingToTransferToken = anotherBuyerTryingToTransferResponse.json<{ token: string }>().token
  ...
  void t.test('not be able to transfer to a user who already had the event', async t => {
    try {
      const response = await app.inject({
        method: 'post',
        url: `/transactions/${transactionIdOfFirstUser.rows[0].transaction_id}/transfer`,
