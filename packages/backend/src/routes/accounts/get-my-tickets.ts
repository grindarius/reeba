...
export default async (instance: FastifyInstance, _: FastifyPluginOptions): Promise<void> => {
  instance.get<{ Params: GetMyTicketsRequestParams, Reply: GetMyTicketsReply }>(
    ...
    async (request) => {
      ...
      type EventIdsThatUsersWentTo = Pick<events, 'event_id' | 'event_name' | 'event_venue_name' | 'user_username' | 'event_status'>
      ...
      & Pick<event_sections, 'event_section_id' | 'event_section_row_position' | 'event_section_column_position'>

      const eventIdsThatUsersWentTo = await instance.pg.query<EventIdsThatUsersWentTo, [events['user_username']]>(
        `select
          events.event_id,
          events.event_name,
      ...
      const groupedByEventId = Object.values(groupBy(eventIdsThatUsersWentTo.rows, (e) => e.event_id))
        .map(eventGroup => {
          return {
            ...
            totalPrice: eventGroup.reduce((total, current) => current.event_seat_price + total, 0)
          }
        })
