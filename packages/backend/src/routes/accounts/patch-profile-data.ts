...
export default async (
  ...
  instance.patch<{
    Params: PatchProfileDataRequestParams
    Body: PatchProfileDataRequestBody
    Reply: PatchProfileDataReply
  }>(
    "/:username/profile-data",
    {
      ...
      preValidation: (request, reply) => {
        ...
        const { email, password, phoneCountryCode, phoneNumber, birthdate } =
          request.body
      ...
      return await instance.pg.transact<PatchProfileDataReply>(async client => {
        ...
        if (password !== "") {
          ...
          const isSamePassword = await verify(
            oldPasswordHash.rows[0].user_password,
            password,
            argon2Options
          )
