...
const plugin = (instance, opts, done) => {
  const getFormattedRoutes = () => {
    return [...instance.routes.values()].flat(1).map(r => {
      return {
        ...
        url: r.url,
        method: typeof r.method === 'string' ? r.method.toLowerCase() : r.method.map(m => m.toLowerCase())
      }
    })
  }
  ...
  const createRoutesString = (r) => {
    ...
    const functionGenerics = argsArray.map(p => `${p.replace(':', '')}: string`).join(', ')
    const functionArgs = argsArray.map(p => p.replace(':', '')).join(', ')
    const urlPaths = pathSegments.map(p => {
  ...
  const writeRoutesToFile = (blacklistRoutes) => {
    ...
    const routes = getFormattedRoutes().flatMap(r => blacklistRoutes.includes(r.name) ? [] : createRoutesString(r))
